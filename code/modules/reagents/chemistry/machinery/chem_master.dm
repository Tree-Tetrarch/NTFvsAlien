		...
		if (href_list["analyze"])
			...
			if(!condi)
				...
				if(ispath(reag_path, /datum/reagent/blood))
					...
					dat += "Chemical infos:<BR><BR>Name:<BR>[A]<BR><BR>Description:<BR>Blood Type: [B]<br>DNA: [C]<BR><BR><BR><A href='byond://?src=[text_ref(src)];main=1'>(Back)</A>"
				else
					dat += "Chemical infos:<BR><BR>Name:<BR>[href_list["name"]]<BR><BR>Description:<BR>[href_list["desc"]]<BR><BR><BR><A href='byond://?src=[text_ref(src)];main=1'>(Back)</A>"
			else
				dat += "Condiment infos:<BR><BR>Name:<BR>[href_list["name"]]<BR><BR>Description:<BR>[href_list["desc"]]<BR><BR><BR><A href='byond://?src=[text_ref(src)];main=1'>(Back)</A>"
			...
			while (count--)
				...
				if(loaded_pill_bottle)
					if(length(loaded_pill_bottle.contents) < loaded_pill_bottle.storage_datum.max_storage_space)
						loaded_pill_bottle.storage_datum.handle_item_insertion(P, TRUE)
						updateUsrDialog()
...
/obj/machinery/chem_master/interact(mob/user)
	...
	if(!beaker)
		...
		if(loaded_pill_bottle)
			dat += "<A href='byond://?src=[text_ref(src)];ejectp=1'>Eject Pill Bottle \[[length(loaded_pill_bottle.contents)]/[loaded_pill_bottle.storage_datum.max_storage_space]\]</A><BR><BR>"
		else
			dat += "No pill bottle inserted.<BR><BR>"
	else
		dat += "<A href='byond://?src=[text_ref(src)];eject=1'>Eject beaker and Clear Buffer</A><BR>"
		if(loaded_pill_bottle)
			dat += "<A href='byond://?src=[text_ref(src)];ejectp=1'>Eject Pill Bottle \[[length(loaded_pill_bottle.contents)]/[loaded_pill_bottle.storage_datum.max_storage_space]\]</A><BR><BR>"
		else
			dat += "No pill bottle inserted.<BR><BR>"
		if(!beaker.reagents.total_volume)
			...
			for(var/datum/reagent/G in beaker.reagents.reagent_list)
				...
				dat += "<A href='byond://?src=[text_ref(src)];analyze=1;desc=[G.description];name=[G.name];reag_type=[G.type]'>(Analyze)</A> "
				dat += "<A href='byond://?src=[text_ref(src)];add=[G.type];amount=1'>(1)</A> "
				dat += "<A href='byond://?src=[text_ref(src)];add=[G.type];amount=5'>(5)</A> "
				dat += "<A href='byond://?src=[text_ref(src)];add=[G.type];amount=10'>(10)</A> "
				dat += "<A href='byond://?src=[text_ref(src)];add=[G.type];amount=[G.volume]'>(All)</A> "
				dat += "<A href='byond://?src=[text_ref(src)];addcustom=[G.type]'>(Custom)</A><BR>"

		dat += "<HR>Transfer to <A href='byond://?src=[text_ref(src)];toggle=1'>[(!mode ? "disposal" : "beaker")]:</A><BR>"
		if(reagents.total_volume)
			for(var/datum/reagent/N in reagents.reagent_list)
				dat += "[N.name] , [N.volume] Units - "
				dat += "<A href='byond://?src=[text_ref(src)];analyze=1;desc=[N.description];name=[N.name];reag_type=[N.type]'>(Analyze)</A> "
				dat += "<A href='byond://?src=[text_ref(src)];remove=[N.type];amount=1'>(1)</A> "
				dat += "<A href='byond://?src=[text_ref(src)];remove=[N.type];amount=5'>(5)</A> "
				dat += "<A href='byond://?src=[text_ref(src)];remove=[N.type];amount=10'>(10)</A> "
				dat += "<A href='byond://?src=[text_ref(src)];remove=[N.type];amount=[N.volume]'>(All)</A> "
				dat += "<A href='byond://?src=[text_ref(src)];removecustom=[N.type]'>(Custom)</A><BR>"
		else
			dat += "Empty<BR>"
		if(!condi)
